---
name: triage
description: "Generate a prioritized triage dashboard document (markdown + HTML) for your open issues with reactions, release context, and discussion links"
agent: issue-tracker
tools:
  - github/*
  - createFile
...
${input:scope:Optional: repo name, label filter, or date range}

## Steps

1. Get the authenticated user with #tool:mcp_github_github_get_me.
2. Search for open issues created by or assigned to the user with #tool:mcp_github_github_search_issues. Apply any scope filter provided.
