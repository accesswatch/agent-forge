---
name: team-dashboard
description: "Team-wide activity dashboard — review load, contribution metrics, bottlenecks, and load balancing insights in dual markdown + HTML format"
agent: analytics
tools:
  - github/*
  - createFile
...
${input:scope:Optional: repo name, date range (e.g. 'last 2 weeks', 'this month'), or specific focus (e.g. 'review load', 'bottlenecks')}

## Steps

1. Get the authenticated user with #tool:mcp_github_github_get_me.
2. Load team roster from `.github/agents/preferences.md` if available.
